...
# Fungsi untuk memvalidasi format tanggal secara manual (dd/mm/yyyy)
def validasi_tanggal(tanggal):
    # Memastikan panjang string tepat 10 karakter dan format sesuai dengan dd/mm/yyyy
    if len(tanggal) != 10 or tanggal[2] != '/' or tanggal[5] != '/':
        return False  # Format tanggal tidak valid, kembalikan False

    # Memisahkan bagian hari, bulan, dan tahun dari string tanggal
    hari = tanggal[:2]
    bulan = tanggal[3:5]
    tahun = tanggal[6:]

    # Memastikan hari, bulan, dan tahun adalah angka
    if not (hari.isdigit() and bulan.isdigit() and tahun.isdigit()):
        return False  # Jika salah satu bukan angka, kembalikan False

    # Memeriksa panjang karakter tahun
    if len(tahun) != 4:
        return False  # Tahun harus memiliki panjang 4 karakter

    # Konversi hari, bulan, dan tahun ke tipe integer
    hari = int(hari)
    bulan = int(bulan)
    tahun = int(tahun)

    # Validasi bulan (harus antara 1 dan 12)
    if bulan < 1 or bulan > 12:
        return False  # Bulan tidak valid, kembalikan False

    # Validasi jumlah hari berdasarkan bulan
    if bulan in [1, 3, 5, 7, 8, 10, 12]:  # Bulan dengan 31 hari
        if hari < 1 or hari > 31:
            return False  # Hari tidak valid untuk bulan ini
    elif bulan in [4, 6, 9, 11]:  # Bulan dengan 30 hari
        if hari < 1 or hari > 30:
            return False  # Hari tidak valid untuk bulan ini
    elif bulan == 2:  # Februari
        if hari < 1 or hari > 29:  # Diasumsikan mengabaikan tahun kabisat
            return False  # Hari tidak valid untuk Februari

    # Validasi tahun (berada dalam rentang yang masuk akal)
    if tahun < 1900 or tahun > 2100:
        return False  # Tahun tidak valid, kembalikan False
